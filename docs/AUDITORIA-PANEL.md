...
| **5** 🟡 | Citas hechas (015). **Auditoría y retención hechas (018)**: `audit_logs` de solo-escritura-por-función con las 5 acciones sensibles conectadas (factura emitida/cobrada, cobro, certificado, olvido RGPD), y retención automática nocturna (IPs 90 días, leads perdidos 2 años, HTML de correos 1 año). **Pantalla del registro de actividad hecha** (Configuración → Registro de actividad, primera página en TypeScript) y **RAT hecho** (`docs/RGPD-RAT.md`, con los plazos que ejecuta la 018). Quedan: **verificación en dos pasos (MFA)** — pedida por Manel, Supabase la soporta con TOTP — y la declaración responsable del productor (va con la fase 6). | Bajo | Casi |
| **4** 🟡 | Reorganizar el front. Hecho: ESLint con `jsx-no-undef`, code splitting (1,59 MB → ~670 kB inicial), desplegable único, `ProjectDetail` partido (2.251 → 1.845 líneas), todas las páginas en `features/<dominio>/pages/`, y la base TypeScript: `tsconfig` en modo convivencia, tipos del esquema real en `database.types.ts`, cliente Supabase tipado y `npm run check` (lint + typecheck + build). Queda: migrar los `.jsx` a `.tsx` gradualmente. El renombrado a castellano (4g) está hecho en la migración 017: 15 tablas renombradas, vistas de compatibilidad para la ventana de deploy, y `users`/`leads`/`facturas`/`blog_posts` conservadas a propósito para no tocar ni la web ni las Edge Functions. | Medio, mecánico | Casi |
